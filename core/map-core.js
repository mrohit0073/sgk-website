...
(function(){
  ...
  // Note: CSS for badges is now in assets/css/index.css

  function checkGlobalStatus(zones){
    const statusBox = document.getElementById('global-status-container');
    if(!statusBox) return;
    let outageCount = 0, firstDownIndex = -1;
    zones.forEach((z, idx) => { if(z.status === 'down'){ outageCount++; if(firstDownIndex===-1) firstDownIndex = idx; } });
    ...
    statusBox.onclick = function(){
      ...
      if(outageCount>0 && firstDownIndex!==-1 && map){
        ...
        setTimeout(()=>{ showZoneInfo(null, firstDownIndex); }, 800);
      }
    };
  ...
  function initMap(zones){
    ...
    zones.forEach((zone, index) => {
      ...
      let polyColor = zone.color; let className = '';
      const statusIcon = zone.status === 'down'
        ? '<i class="fas fa-exclamation-triangle text-red-500 zone-badge__icon"></i>'
        : '<i class="fas fa-check-circle text-green-400 zone-badge__icon"></i>';
      if (zone.status === 'down'){ polyColor = '#ef4444'; className = 'zone-down'; }
      const polygon = L.polygon(zone.points, { color: polyColor, fillColor: polyColor, fillOpacity: 0.15, weight: 2, className }).addTo(map);
      const tooltipContent = `<div class="zone-badge" onclick="showZoneInfo(event, ${index})">${statusIcon}<span class="zone-badge__text">${Utils.escapeHtml(zone.name)}</span></div>`;
      polygon.bindTooltip(tooltipContent, { permanent: true, direction: 'center', interactive: true, className: 'zone-badge-tooltip', opacity: 0.95 });
      polygon.on('click', function(e){ handleMapClick(e); });
    });
  ...
  window.showZoneInfo = function(e, index){
    ...
    const zone = zoneDataRef[index]; if (!zone) return;
    const poly = L.polygon(zone.points); const center = poly.getBounds().getCenter();
    let content = '';
    if (zone.status === 'down'){
      content = `
        <div class="text-center p-2 min-w-[200px]">
          ...
          <div class="font-bold text-white mb-2">${Utils.escapeHtml(zone.name)}</div>
          <div class="bg-red-500/10 border border-red-500/20 p-2 rounded text-left text-xs space-y-1">
            <div class="text-gray-400">Reason: <span class="text-red-500">${Utils.escapeHtml(zone.issue || 'Maintenance')}</span></div>
            <div class="text-gray-400">ETA: <span class="text-red-500 font-bold">${Utils.escapeHtml(zone.eta || 'Calculating...')}</span></div>
          </div>
        </div>`;
    } else {
      content = `
        <div class="text-center p-2 min-w-[150px]">
          ...
          <div class="font-bold text-white text-sm">${Utils.escapeHtml(zone.name)}</div>
          <div class="text-xs text-gray-500 mt-1">All services operational</div>
        </div>`;
    }
    L.popup({ className: 'glass-popup' }).setLatLng(center).setContent(content).openOn(map);
  }
  ...
  function locateUser(){
    ...
    icon.classList.replace('fa-location-crosshairs', 'fa-spinner'); icon.classList.add('fa-spin');
    navigator.geolocation.getCurrentPosition(
      (pos) => { icon.classList.replace('fa-spinner', 'fa-location-crosshairs'); icon.classList.remove('fa-spin'); document.getElementById('locationInput').value = 'My Current Location'; handleLocationSelect(pos.coords.latitude, pos.coords.longitude, 'Your Location'); },
      () => { icon.classList.replace('fa-spinner', 'fa-location-crosshairs'); icon.classList.remove('fa-spin'); alert('Unable to get location.'); }
    );
  }
  ...
  function setupSearch(){
    ...
    input.addEventListener('input', (e) => {
      ...
      debounce = setTimeout(async () => {
        ...
        } catch(e){} finally { if (icon) icon.className = 'fas fa-search absolute left-4 top-5 text-gray-500 z-20'; }
      }, 200);
    });
  ...
  function displaySuggestions(features){
    ...
    features.forEach(item => {
      ...
      const label = Utils.escapeHtml(item.properties.name || item.properties.street || 'Unknown');
      div.innerHTML = `<i class="fas fa-map-marker-alt"></i> <span>${label}</span>`;
      div.onclick = () => { input.value = div.innerText; box.classList.remove('active'); handleLocationSelect(item.geometry.coordinates[1], item.geometry.coordinates[0], div.innerText); };
      box.appendChild(div);
    });
  }
  ...
  function handleLocationSelect(lat, lng, name){
    ...
    let isCovered = false;
    coveragePolygons.forEach(poly => { if (isPointInPolygon([lat, lng], poly)) isCovered = true; });
    const color = isCovered ? 'marker-cyan' : 'marker-red';
    const icon = L.divIcon({ className: 'custom-pin', html: `<div class="marker-container"><div class="marker-pulse ${color} ${isCovered?'marker-glow-cyan':'marker-glow-red'}"></div><div class="marker-dot ${color}"></div></div>`, iconSize: [20, 20] });
  ...
  window.MapCore = { initMap, checkGlobalStatus };
})();
