...
// Inject compact badge CSS once (for Leaflet tooltips on zones)
function ensureZoneBadgeStyles() {
  if (document.getElementById('zone-badge-style')) return;
  const css = `
    .leaflet-tooltip.zone-badge-tooltip {
    ...
    .zone-badge:hover { transform: scale(1.05); }
    .zone-badge__icon { font-size: 0.875rem; line-height: 1; }
    .zone-badge__text {
      font-size: 0.675rem;
      font-weight: 800;
  ...
  style.id = 'zone-badge-style';
  style.type = 'text/css';
  style.appendChild(document.createTextNode(css));
  document.head.appendChild(style);
}

// Simple HTML escaper for zone names
...
function initMap(zones) {
  ...
  // Ensure badge styles exist (no index.html edits required)
  ensureZoneBadgeStyles();

  // Store zones globally so the tooltip click handler can access them
  zoneDataRef = zones;

  // Reset coverage set to avoid duplicates after re-init
  coveragePolygons = [];
  ...
  zones.forEach((zone, index) => {
    ...
    // --- INTERACTIVE TOOLTIP (COMPACT BADGE) ---
    const tooltipContent =
      `<div class="zone-badge" onclick="showZoneInfo(event, ${index})">` +
        `${statusIcon}` +
        `<span class="zone-badge__text">${escapeHtml(zone.name)}</span>` +
      `</div>`;
  ...
  if (zone.status === 'down') {
    content = `
      <div class="text-center p-2 min-w-[200px]">
        ...
        <div class="font-bold text-white mb-2">${escapeHtml(zone.name)}</div>
        <div class="bg-red-500/10 border border-red-500/20 p-2 rounded text-left text-xs space-y-1">
          <div class="text-gray-400">Reason: <span class="text-red-500">${escapeHtml(zone.issue || 'Maintenance')}</span></div>
          <div class="text-gray-400">ETA: <span class="text-red-500 font-bold">${escapeHtml(zone.eta || 'Calculating...')}</span></div>
        </div>
      </div>
    `;
  } else {
    content = `
      <div class="text-center p-2 min-w-[150px]">
        ...
        <div class="font-bold text-white text-sm">${escapeHtml(zone.name)}</div>
...
function displaySuggestions(features) {
  ...
  features.forEach(item => {
    ...
    div.innerHTML = `<i class="fas fa-map-marker-alt"></i> <span>${escapeHtml(item.properties.name || item.properties.street || "Unknown")}</span>`;
    div.onclick = () => {
